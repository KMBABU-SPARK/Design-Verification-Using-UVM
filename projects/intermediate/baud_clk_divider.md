# UVM Testbench for Clock Generator / Baud Clock Divider

A simple **UVM (Universal Verification Methodology)** testbench that verifies a **clock generator (baud clock divider)** using a complete UVM environment: sequence item, sequences, driver, monitor, scoreboard, agent, environment, and test.
...
3. Check the log for `SYSTEM RESET DETECTED`, `TEST PASSED`, or `TEST FAILED`.

## Notes

* The `transaction` class stores the baud value and measured period.
* The `driver` applies reset or baud settings to the DUT.
* The `monitor` measures the `tx_clk` period using simulation time.
* The `scoreboard` compares the measured count with the expected baud divider value.
* The `test` currently starts only `variable_baud`; `reset_clk` is created but not started.
